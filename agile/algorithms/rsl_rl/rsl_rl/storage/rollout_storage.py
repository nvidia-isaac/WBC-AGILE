...
class RolloutStorage:
    ...
    def __init__(
        ...
        obs_shape: int | TensorDict,
        privileged_obs_shape: int | TensorDict,
        actions_shape,
        ...
        # Core
        if isinstance(obs_shape, TensorDict):
            self.observations = (
                obs_shape.clone()
                .unsqueeze(0)
                .unsqueeze(0)
                .repeat(num_transitions_per_env, num_envs)
            )
        else:
        ...
        if privileged_obs_shape is not None:
            if isinstance(privileged_obs_shape, TensorDict):
                self.privileged_observations = (
                    privileged_obs_shape.clone()
                    .unsqueeze(0)
                    .unsqueeze(0)
                    .repeat(num_transitions_per_env, num_envs)
                )
    ...
    def mini_batch_generator(self, num_mini_batches, num_epochs=8):
        ...
        if self.use_l2c2:
            prev_obs = self.observations[:-1].flatten(0, 1)
            next_obs = self.observations[1:].flatten(0, 1)
            if self.privileged_observations is not None:
                prev_priv_obs = self.privileged_observations[:-1].flatten(0, 1)
                next_priv_obs = self.privileged_observations[1:].flatten(0, 1)
            else:
                prev_priv_obs = prev_obs
                next_priv_obs = next_obs
            l2c2_done_mask = self.dones[:-1].squeeze().bool().flatten(0, 1)
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                if self.use_l2c2:
                    ...
                    l2c2_end = (i + 1) * l2c2_mini_batch_size
                    l2c2_batch_idx = l2c2_indices[l2c2_start:l2c2_end]
                    prev_obs_batch = prev_obs[l2c2_batch_idx]
                    next_obs_batch = next_obs[l2c2_batch_idx]
                    if self.privileged_observations is not None:
                        prev_priv_obs_batch = prev_priv_obs[l2c2_batch_idx]
                        next_priv_obs_batch = next_priv_obs[l2c2_batch_idx]
                    else:
                        prev_priv_obs_batch = prev_obs_batch
                        next_priv_obs_batch = next_obs_batch
                    done_mask_batch = l2c2_done_mask[l2c2_batch_idx]
                    l2c2_batch = (prev_obs_batch, next_obs_batch, prev_priv_obs_batch, next_priv_obs_batch, done_mask_batch)
                else:
                    l2c2_batch = None
                ...
                yield obs_batch, privileged_observations_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch, old_mu_batch, old_sigma_batch, (
                    None,
                    None,
                ), None, rnd_state_batch, l2c2_batch
    ...
    def recurrent_mini_batch_generator(self, num_mini_batches, num_epochs=8):
        ...
        if self.use_l2c2:
            raise NotImplementedError("L2C2 regularization is not implemented for recurrent mini-batch generator.")
        ...
        for ep in range(num_epochs):
            ...
            for i in range(num_mini_batches):
                ...
                yield obs_batch, privileged_obs_batch, actions_batch, values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch, old_mu_batch, old_sigma_batch, (
                    hid_a_batch,
                    hid_c_batch,
                ), masks_batch, rnd_state_batch, None
