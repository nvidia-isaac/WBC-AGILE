...
class RandomNetworkDistillation(nn.Module):
    ...
    def __init__(
        ...
        self.predictor = self._build_mlp(
            num_states, predictor_hidden_dims, num_outputs, activation
        ).to(self.device)
        self.target = self._build_mlp(
            num_states, target_hidden_dims, num_outputs, activation
        ).to(self.device)
        ...
        for layer_index in range(len(hidden_dims)):
            if layer_index == len(hidden_dims) - 1:
                ...
                network_layers.append(
                    nn.Linear(hidden_dims[layer_index], hidden_dims[layer_index + 1])
                )
                network_layers.append(activation)
