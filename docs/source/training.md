...
AGILE uses the same training script interface as Isaac Lab. Run `uv run scripts/train.py -h` for all available arguments.

## Training

```bash
uv run scripts/train.py --task Velocity-T1-v0 --num_envs 4096 --headless --logger wandb
```
...
```bash
uv run scripts/play.py --task StandUp-T1-v0 --num_envs 16 --validate-fallen-states
```
...
uv run scripts/wandb_sweep/init_sweep.py --project_name my-sweep

# 3. Start agents (run on each GPU/machine)
uv run scripts/wandb_sweep/run_sweep.py --project_name my-sweep --agent_count 10
```
...
# Export through Isaac Lab's LEAPP exporter
