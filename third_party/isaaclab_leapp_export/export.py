...
VENDORED FROM ISAAC LAB. This is a verbatim copy (with only the ``cli_args`` import path
adjusted, see below) of Isaac Lab's LEAPP export driver:

    scripts/reinforcement_learning/leapp/rsl_rl/export.py

Source: isaac-sim/IsaacLab @ v3.0.0 (commit 22c6b0397feb445c8785539a3eaf201bcc2d9939).

Isaac Lab ships this driver only in its repository ``scripts/`` tree, which is NOT part of
any installable wheel. AGILE installs Isaac Lab via uv as wheels (``isaaclab==3.0.0b2``),
so the driver is absent from the environment. The LEAPP *library* it calls
(``isaaclab.utils.leapp``, ``isaaclab_rl.rsl_rl``, ``leapp``) IS in the wheels, so vendoring
this thin driver lets ``scripts/export_policy_leapp.py`` run from a pure uv install without
requiring an Isaac Lab source checkout / ``ISAACLAB_PATH``.

To update: re-copy from the matching Isaac Lab tag and re-apply both AGILE changes: (1) the
``cli_args`` import-path tweak, and (2) the optional ``on_env_ready`` hook (the kwarg on
``export_rsl_rl_agent`` plus the call right after ``RslRlVecEnvWrapper`` is built).
"""
...
# ``cli_args`` is vendored alongside this module (in the upstream layout it lives two
# directories up under ``rsl_rl/``).
...
def export_rsl_rl_agent(
    ...
    agent_cfg,
    simulation_app=None,
    on_env_ready=None,  # AGILE ADDITION: optional callback(env) invoked once the (wrapped) env is built, before export. Lets callers adjust the live env (e.g. populate gain buffers) prior to LEAPP annotation.
    checkpoint_load_cfg=None,  # AGILE ADDITION: optional rsl_rl load selection for inference-only export.
) -> bool:
    ...
    try:
        ...
        # AGILE ADDITION: let the caller adjust the live env before the policy/graph is annotated.
        ...
        runner.load(resume_path, load_cfg=checkpoint_load_cfg)
