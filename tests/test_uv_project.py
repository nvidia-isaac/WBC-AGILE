...
class TestUvProject(unittest.TestCase):
    ...
    def test_ci_lints_internal_and_generated_public_release_surface(self) -> None:
        ci_path = _REPO_ROOT / ".gitlab-ci.yml"
        if not ci_path.is_file():
            self.skipTest("Internal GitLab CI configuration is not part of the Public Release Surface")
        ci_config = ci_path.read_text()

        self.assertIn(".public_export:", ci_config)
        self.assertIn("public_pre_commit:", ci_config)
        self.assertEqual(ci_config.count("extends: .public_export"), 2)
        self.assertEqual(ci_config.count("pre-commit run --all-files"), 2)
        self.assertIn("git init", ci_config)
        self.assertIn("git add -f -A", ci_config)

    def test_github_gpu_tests_are_manual_and_use_current_pytest_entrypoints(self) -> None:
        workflows = _REPO_ROOT / ".github" / "workflows"
        premerge = (workflows / "premerge.yml").read_text()
        build_and_test = (workflows / "build-and-test.yml").read_text()

        self.assertNotIn("build_and_test:", premerge)
        self.assertIn("workflow_dispatch:", build_and_test)
        self.assertNotIn("pull_request:", build_and_test)
        self.assertIn('uv run --frozen pytest -m "not e2e"', build_and_test)
        self.assertIn("uv run --frozen pytest -m e2e", build_and_test)
